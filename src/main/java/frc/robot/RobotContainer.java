...
public class RobotContainer {
	...
	public Command generateAutoDriveCommand(){
		return driveSubsystem.autoGeneratedPath(
			()->mlVision.getClosestNotePos(), ()->intakeSubsystem.hasNote()).until(()->intakeSubsystem.hasNote())
			.alongWith(intakeNote().repeatedly()).until(()->intakeSubsystem.hasNote());
	}
	...
	public void generateNamedCommands(){
		...
		NamedCommands.registerCommand("GenerateAutoDrive", new InstantCommand(()->{autoDrive = generateAutoDriveCommand();}));

		NamedCommands.registerCommand("DriveToNote", new InstantCommand(()->autoDrive.schedule()));

		NamedCommands.registerCommand("IntakeWait", new WaitUntilCommand(()->intakeSubsystem.hasNote()));
