...
public class DriveSubsystem extends SubsystemBase {
    ...
    public Command autoGeneratedPath(Supplier<Translation2d> endState){
        List<Translation2d> bezierPoints = PathPlannerPath.bezierFromPoses(
        new Pose2d(getPose().getX(), getPose().getY(), 
            new Rotation2d(Math.atan2(getPose().getX() - endState.get().getX(), getPose().getY() - endState.get().getY()))),
        new Pose2d(endState.get().getX(), endState.get().getY(), new Rotation2d(Math.atan2(getPose().getX() - endState.get().getX(), getPose().getY() - endState.get().getY())))
        );

        PathPlannerPath path = new PathPlannerPath(
            bezierPoints,
            new PathConstraints(3.0, 3.0, 2 * Math.PI, 4 * Math.PI),
            new GoalEndState(0.0, new Rotation2d(Math.atan2(getPose().getX() - endState.get().getX(), getPose().getY() - endState.get().getY()) - Math.PI)));
